...
class Processed:
    def __init__(self, p: StableDiffusionProcessing, images_list, seed=-1, info="", subseed=None, all_prompts=None, all_negative_prompts=None, all_seeds=None, all_subseeds=None, index_of_first_image=0, infotexts=None):
        ...
        self.sd_model_hash = p.sd_model.sd_model_hash
...
def create_infotext(p, all_prompts, all_seeds, all_subseeds, comments, iteration=0, position_in_batch=0):
    ...
    generation_params = {
        ...
        "Model hash": getattr(p, 'sd_model_hash', None if not opts.add_model_hash_to_info or not p.sd_model.sd_model_hash else p.sd_model.sd_model_hash),
        "Model": (None if not opts.add_model_name_to_info or not p.sd_model.sd_checkpoint_info.model_name else p.sd_model.sd_checkpoint_info.model_name.replace(',', '').replace(':', '')),
    ...
    with torch.no_grad(), p.sd_model.ema_scope():
        ...
        for n in range(p.n_iter):
            ...
            with devices.autocast():
                ...
                c = prompt_parser.get_multicond_learned_conditioning(p.sd_model, prompts, p.steps)
